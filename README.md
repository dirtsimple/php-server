...
This image assumes your primary application code/git checkout will be found in `/var/www/html`.  If you supply a `GIT_REPO` environment variable, it will be cloned there at container startup unless a `.git` subdirectory is already present.  Alternately, you can supply your code via a volume mount or by installing it there in a derived image.

After the automatic cloning process (if applicable and successful), this image checks for the following things in the `/var/www/html` directory:

* a `composer.lock` file (triggering an automatic `composer install` run)
* a `conf-tpl/` subdirectory (triggering configuration file updates from supplied templates -- see next section for details)
...
This image uses [gomplate](https://github.com/hairyhenderson/gomplate) to generate arbitrary configuration files from templates.

Templates are loaded from two locations:

* The `/tpl` directory created at build-time, supplied by this image
* The `/var/www/html/conf-tpl` directory, found in your code checkout, volume mount, or derived image

The path of a configuration file is derived from its relative path.  So, for example, the default template for `/etc/supervisord.conf` can be found in `/tpl/etc/supervisord.conf`, and can be overrridden by a template in `/var/www/html/conf-tpl/etc/supervisord.conf`.
...
(Note: unlike ngineered/nginx-php-fpm, this image does *not* support a plain `conf` directory with nginx configuration files. If you need to change the nginx configuration, we suggest simply defining a complete nginx configuration template in your code's `conf-tpl/etc/nginx/nginx.conf` directory.)
